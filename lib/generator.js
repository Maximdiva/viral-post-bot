...
export const REGIONS = [
  {
    id: 'NG',
    name: 'Nigeria',
    voice: `You grew up in Lagos. You speak Pidgin when it fits naturally. You know the streets, the music scene, Nollywood drama, EFCC wahala, sapa life, CBN policies, and Afrobeats beef. You know Burna Boy, Wizkid, Davido, Portable, Bobrisky, VDM, Shallipopi. You reference ASUU, PVC, Aso Rock naturally.`,
    peakHoursUTC: [7, 8, 9, 11, 12, 18, 19, 20],
  },
  {
    id: 'US',
    name: 'United States',
    voice: `You are terminally online. You know NBA beef, NFL takes, pop culture drama, political chaos, TikTok sounds, hip hop feuds, tech layoffs, Wall Street moves. You say things like "bro what", "this is insane", "no way this is real", "they cooked", "the audacity". You know Drake, Kendrick, Taylor Swift, Trump, LeBron, Elon, Zuckerberg energy.`,
    peakHoursUTC: [12, 13, 14, 20, 21, 22, 23],
  },
  {
    id: 'IN',
    name: 'India',
    voice: `You live on Twitter India. You know IPL drama, Bollywood fights, UPSC pain, startup culture, political news, reels trends, influencer beef. You say things like "this guy is something else", "absolute scenes", "only in India", "the audacity of this man". You know Kohli, Rohit, Shah Rukh, Ranveer, Modi, Kejriwal, the startup bros.`,
...
TWEET 1 (hook):
A punchy, information-dense sentence or two explaining exactly why "${topicName}" is trending, what happened, and why people care. End with: "See full trend summary in the thread [down arrow][thread emoji]"
...
Each tweet starts with its number: 1/ 2/ 3/ 4/ 5/
...
- Body tweets use plain numbers: 1/ 2/ 3/ 4/ 5/
- Minimum 3 body tweets, maximum 5
- "END OF THREAD." must be the last line of the final tweet
- Use only standard ASCII characters in your response - no special unicode
...
["hook tweet", "summary tweet 1", "summary tweet 2", "summary tweet 3"]`
  ...
  try {
    const response = await groq.chat.completions.create({
      model: MODEL,
      messages: [
        { role: 'system', content: 'You are a viral social media writer. Return ONLY a valid JSON array of strings. No markdown, no backticks. Use plain "1/" "2/" format for numbering. Use only standard ASCII characters.' },
    ...
    try {
      ...
      console.error('[THREAD] JSON parse failed, attempting manual extract')
      const matches = raw.match(/"([^"\\]|\\.)*"/g)
      if (!matches || matches.length < 2) throw new Error('Could not extract tweets')
...
export async function generateThread(region, slot) {
  ...
  return {
    ...
    tweets,
    tweetsFormatted: tweets.map((t, i) => `<p><b>Tweet ${i + 1}:</b><br>${t.replace(/\n/g, '<br>')}</p>`).join('<hr>'),
