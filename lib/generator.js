...
Each tweet starts with its emoji number: 1/ 2/ 3/ 4/ 5/
...
- Body tweets use emoji numbers: 1/ 2/ 3/ 4/ 5/
...
["hook tweet", "summary tweet 1", "summary tweet 2", "summary tweet 3"]`
  ...
  try {
    const response = await groq.chat.completions.create({
      model: MODEL,
      messages: [
        { role: 'system', content: 'You are a viral social media writer. Return ONLY a valid JSON array of strings. No markdown, no backticks, no emoji numbers like 1️⃣. Use plain "1/" "2/" format instead.' },
    ...
    // Strip everything that could break JSON
    raw = raw
      .replace(/```json|```/g, '')
      .replace(/[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F]/g, '')
      .replace(/[\u{1F100}-\u{1F1FF}]/gu, '') // strip enclosed number emojis like 1️⃣
      .trim()
    ...
    try {
      ...
      // Last resort: try to extract strings manually
